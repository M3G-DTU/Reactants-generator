...
def molecule_info(out_file):
    openold = open(out_file,"r", encoding='utf-8', errors='replace')
    rline = openold.readlines()
    for l in range (len(rline)):
        if "C O N C E P T U A L   D F T   ( F D L )" in rline[l]:
            head = l
    for i in range (head, len(rline)):
...
def fukui_info(out_file):
    ...
    for l in range (len(rline)):
        if "C O N C E P T U A L   D F T   ( F D L )" in rline[l]:
            head = l
    for i in range (head, len(rline)):
        if "MAIN ATOMIC DESCRIPTORS: CANONICAL ENSEMBLE" in rline[i]:
            start = i+5
    for m in range (start, len(rline)):
        if "Total" in rline[m]:
            end = m-1
...
def elec_filter(list,fukui_list):
    ...
    for atom in sorted(list,key=(lambda x:float(x[-2])),reverse=True):
        if float(atom[-2])>0:
            if float(atom[-2])+float(atom[-1])>0:
...
def nuc_filter(list,fukui_list):
    filtrate=[]
    for atom in sorted(list,key=(lambda x:float(x[-1]))):
        if float(atom[-1])<0:
            if float(atom[-2])+float(atom[-1])<=0:
                filtrate.append(atom)
            else:
...
print([i[0] for i in out1_eletro_high2low])
# 2
...
print([i[0] for i in out1_nucleo_low2high])
# 3
...
print([i[0] for i in out2_eletro_high2low])
# 4
...
print([i[0] for i in out2_nucleo_low2high])
...
for atom in out1_eletro_high2low:
    atom_label, hot1 = re.match(r'([A-Za-z]+)\((\d+)\)', atom[0]).groups(); hot1 = int(hot1)
    if float(atom[-2])>= float(out1_eletro_high2low[0][-2])-elec_cutoff:
        for atom2 in out2_nucleo_low2high:
            atom2_label, hot2 = re.match(r'([A-Za-z]+)\((\d+)\)', atom2[0]).groups(); hot2 = int(hot2)
            if float(atom2[-1]) <= float(out1_nucleo_low2high[0][-1])+nucl_cutoff:
                produce_script=('python molecule_rearrangement.py '+str(xyz_file1)+' '+str(xyz_file2)+' '+str(hot1)+' '+str(hot2)+' '+str(directory)+'/'+ str(xyz_file1.split('/')[-1].split('.')[0])+'_'+str(xyz_file2.split('/')[-1].split('.')[0])+'_ElecNu_'+str(hot1)+str(atom_label)+'_'+str(hot2)+str(atom2_label)+'.xyz')
                print('-------------------')
...
for atom in out2_eletro_high2low:
    ...
    if float(atom[-2])>= float(out2_eletro_high2low[0][2])-elec_cutoff:
        for atom2 in out1_nucleo_low2high:
            atom2_label, hot2 = re.match(r'([A-Za-z]+)\((\d+)\)', atom2[0]).groups(); hot2 = int(hot2)
            if float(atom2[-1]) <= float(out1_nucleo_low2high[0][-1])+nucl_cutoff:
                produce_script=('python molecule_rearrangement.py '+str(xyz_file2)+' '+str(xyz_file1)+' '+str(hot1)+' '+str(hot2)+' '+str(directory)+'/'+str(xyz_file2.split('/')[-1].split('.')[0])+'_'+str(xyz_file1.split('/')[-1].split('.')[0])+'_ElecNu_'+str(hot1)+str(atom_label)+'_'+str(hot2)+str(atom2_label)+'.xyz')
                print('-------------------')
                print(produce_script.split()[-1])
                produce_file=os.system(produce_script)
            else:
